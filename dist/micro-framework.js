...
(function (global, factory) {
    ...
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.MicroFramework = {}));
    ...
    class MicroFramework {
    	constructor(options = {}) {
    		...
    		this.config = {
    			...
    			autoBindLinks: options.autoBindLinks !== false, // Enable automatic link binding
    			linkSelector: options.linkSelector || "[data-route]", // Simple selector for navigation links
    			...options,
    		};
    		...
    		this.router = new Router(this, {
    			...
    			notFoundHandler:
    				options.router?.notFoundHandler ||
    	...
    	initializeContainer() {
    		...
    		if (typeof this.config.container === "string") {
    			this.moduleContainer = document.querySelector(
    		...
    		this.containerObserver = new MutationObserver((mutations) => {
    			...
    			for (const mutation of mutations) {
    				...
    				for (const removedNode of mutation.removedNodes) {
    					if (
    						removedNode === this.moduleContainer ||
    						(removedNode.contains &&
    							removedNode.contains(this.moduleContainer))
    					) {
    	...
    	attemptContainerRecovery() {
    		try {
    			...
    			if (currentModule && currentModule.render) {
    				console.log(
    					"Re-rendering current module after container recovery"
    				);
    	 ...
    	 */
    	setupEventListeners() {
    		// Handle navigation clicks if auto-binding is enabled
    	...
    	handleLinkClick(event) {
    		const link = event.target.closest(this.config.linkSelector);
    		if (!link) return;
    	...
    	showFallbackError(message, error = null) {
            ...
            <div style="position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); 
                ...
                ${
					error
						? `<pre style="background: #f1f1f1; padding: 10px; border-radius: 3px;">${error.message}</pre>`
						: ""
				}
    	...
    	async filterRaw(event, data = null) {
    		...
    		for (const callback of listeners) {
    			try {
    				...
    				console.error(
    					`Error in filter listener for '${event}':`,
    					error
    				);
    	...
    	destroy() {
    		...
    		try {
    			...
    			console.log(
    				"Container not available during destroy - this is normal"
    			);
    		}
    ...
    exports.default = MicroFramework;

    Object.defineProperty(exports, '__esModule', { value: true });

}));
//# sourceMappingURL=micro-framework.js.map
